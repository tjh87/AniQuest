...
import { ProfileSection, ProfileSectionControls, useProfileSections, type ProfileSectionId } from "./profile-sections";
import { ProfileFieldNotes } from "./profile-field-notes";
...
export function SpeciesLibrary({ query = "", onOpen, onClear }: { query?: string; onOpen: (id: string) => void; onClear?: () => void }) {
  ...
  const hasActiveFilters = Boolean(search.trim() || tag || group !== "All" || encounter !== "All" || conservation !== "All");
  const nativeCount = SINGAPORE_SPECIES.filter((species) => species.origin === "Native").length;
  ...
  return <section className="aq-species-library" aria-labelledby="animal-library-heading">
    <div className="aq-section-head"><h2 id="animal-library-heading">Explore the animal guide</h2><span className="aq-muted">{SINGAPORE_SPECIES.length} complete profiles · {nativeCount} native</span></div>
    <p className="aq-scope-note">This growing vertebrate guide covers mammals, birds, reptiles and amphibians. It is not yet a complete inventory of every Singapore species. Fish and insects remain outside the current scope.</p>
...
export function AnimalAtlas({ selectedId, onSelect, onBrowse, onJourney }: { selectedId: string; onSelect: (id: string) => void; onBrowse: () => void; onJourney?: (id: string) => void }) {
  const sectionControls = useProfileSections();
  ...
  const activeSections: ProfileSectionId[] = ["details", "sources", "related", "habitat", "review"];
  if (species.funFacts.length) activeSections.push("facts");
  if (photo) activeSections.push("photo");
  const sectionProps = (key: ProfileSectionId) => ({ open: sectionControls.sections[key], onOpenChange: (open: boolean) => sectionControls.setSection(key, open) });
  return <div className="aq-view aq-animal-profile">
    ...
    <div className="aq-page-title aq-profile-title"><div><span className="aq-eyebrow">Singapore Animal Atlas</span><h1>{species.name}</h1><p className="aq-scientific">{species.scientific}</p><div className="aq-tags"><span>{species.group}</span><span>{species.origin}</span><span>{species.encounter}</span>{species.tags?.map(tag => <span className={tag === "Migratory" ? "is-migratory" : undefined} key={tag}>{tag}</span>)}</div></div>{species.funFacts.length > 0 && <ProfileSection title="Fun facts" description={`${species.funFacts.length} source-linked facts`} {...sectionProps("facts")}><div className="aq-profile-title-facts">{species.funFacts.map((fact, index) => <AnimalFunFact key={`${species.id}-${fact.title}`} species={species} index={index} compact />)}</div></ProfileSection>}</div>
    ...
    <ProfileSectionControls activeSections={activeSections} controls={sectionControls} />
    <div className="aq-profile-columns">
      <div className="aq-profile-reading">
        <ProfileSection title="Animal details" description="Identification, life, habitat and conservation" className="aq-profile-details" {...sectionProps("details")}>
          <Tabs key={species.id} defaultValue="overview">
            ...
            <TabsContent value="overview"><ProfileFieldNotes species={species} /><h3 className="aq-profile-subheading">Behaviour</h3><p>{species.behaviour}</p>{species.aliases.length > 0 && <div className="aq-aliases"><strong>Also found under</strong><span>{species.aliases.join(" · ")}</span></div>}</TabsContent>
      ...
      <aside className="aq-profile-evidence">
        {photo && <ProfileSection title="Animal photograph" className="aq-profile-photo-section" {...sectionProps("photo")}><div className="aq-profile-media"><WildlifePhoto animal={species.id} zoomable /></div></ProfileSection>}
        <ProfileSection title="Sources & reference pictures" description={`${uniqueSources.length} references · Reviewed ${species.reviewedAt}`} {...sectionProps("sources")}><p className="aq-review-note">Profile review: {species.reviewedAt}.{species.globalReviewedAt ? ` Global reference checked: ${species.globalReviewedAt}.` : ""} Read the linked source for its publication date and scope.</p><div className="aq-profile-source-list">{uniqueSources.map((source) => <article key={source.url}><a href={source.url} target="_blank" rel="noreferrer">{source.name} <ExternalLink aria-hidden="true" /></a><p>{source.supports}</p></article>)}</div>{!photo && <p className="aq-evidence-note">A verified reference photograph is not available for this profile yet.</p>}{species.taxonomyNote && <p className="aq-evidence-note">{species.taxonomyNote}</p>}</ProfileSection>
      </aside>
    </div>
    <ProfileSection title="Related profiles" description="Relationship graph and photo cards" {...sectionProps("related")}><RelatedSpecies key={species.id} species={species} onOpen={onSelect} groupControls={sectionControls} embedded /></ProfileSection>
    ...
    <ProfileSection title="Singapore in focus" description="Local habitat context and source data" {...sectionProps("habitat")}><SingaporeInsights species={species} embedded /></ProfileSection>
    <ProfileSection title="Content record and factual review" description={`${PROFILE_CLAIMS.length} claim checks and their sources`} {...sectionProps("review")}><ContentReviewNote contentId={`profile:${species.id}`} claims={PROFILE_CLAIMS} embedded /></ProfileSection>
  </div>;
}
